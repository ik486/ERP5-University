...
class Bill(XMLObject):
   ...
   factory_type_information = \
     {    'id'             : portal_type
        , 'meta_type'      : meta_type
        , 'description'    : '''A Bill...'''
        , 'icon'           : 'folder_icon.gif' # should create a new bill.gif for the new type...and change folder_icon.gif name
        , 'product'        : 'ERP5'
        , 'factory'        : 'addBill'
        , 'immediate_view' : 'bill_view'
        , 'allow_discussion'     : 1
   ...
   def edit(self, **vals):
   	...
   	if vals.has_key('bill_type'):
   	   self.setBillType(vals['bill_type'])
...
###########################################################################
   ...
   def calculateBudgetItem(self):
		...
		elif not obj.isOKState(): return
		else: obj.calculateAll()
        return
